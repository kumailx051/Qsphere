...
export default function AdminPageShell({ eyebrow, title, titleAccent, description, actions, children }) {
  ...
  useEffect(() => {
    const check = () => {
      try { setSidebarCollapsed(localStorage.getItem('qsphere_admin_sidebar_collapsed') === '1') } catch {}
    }
            ...
            <section className="relative overflow-hidden rounded-[38px] p-7 md:p-10" style={{ border: `1px solid ${palette.borderPrimary}`, background: isDayMode ? 'linear-gradient(145deg, rgba(255,255,255,0.97), rgba(247,247,245,0.88))' : 'linear-gradient(145deg, rgba(255,255,255,0.055), rgba(255,255,255,0.015))', boxShadow: palette.shadowCard }}>
              ...
              <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <h1 className="type-heading-soft mt-4 max-w-5xl" style={{ color: palette.textPrimary }}>
                    {title}{titleAccent ? <span style={{ color: palette.accentPrimary }}> {titleAccent}</span> : null}
                  </h1>
